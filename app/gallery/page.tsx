  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 flex transition-opacity ${settings.transition === "crossfade" ? "duration-1000" : "duration-0"}`}>
        <SlideView celebrity={current} photoBaseUrl={R2_BASE} showFact={false} colorMode />
      </div>
      ...
      {/* Settings panel overlay — with dimmed "Show Facts" */}
      {showSettings && (
        <SettingsPanel settings={settings} onUpdate={update} onClose={() => setShowSettings(false)} disableFacts />
      )}
    </main>
  );
}
